...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    function buildTokens(scheme, p) {
      ...
      const fg = dark ? mix(t1, BLACK, 0.8) : WHITE
      const brandText = dark ? mix(brand, WHITE, 0.7) : brand2
      const borderBase = dark ? WHITE : t1
      const interactive = dark ? WHITE : brand
    ...
    return {
      apply(ctx) {
        ...
        // —— 激活即应用默认主题(静态插件:显式收集 disposer,卸载时统一还原) ——
        let current = 'aurora'
